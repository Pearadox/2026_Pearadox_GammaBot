...
public class DemoAimAtTrashCan extends Command {
  ...
  private static final LoggedTunableNumber driveMaxVel =
      new LoggedTunableNumber("Demo/Max Vel", 1.5);
  private static final LoggedTunableNumber driveMaxAcc =
      new LoggedTunableNumber("Demon/Max Acc", 2);
  ...
  public DemoAimAtTrashCan(Drive drive, Vision vision, Turret turret) {
    ...
    this.targetPoseSupplier =
        () -> DriveHelpers.calculateOffsetInFrontOfTag(vision.getLastTrustedPose());
  ...
  public void execute() {
    ...
    Translation2d direction = distance > 0.01 ? error.div(distance) : new Translation2d(); // 1e-6

    Translation2d velocity = direction.times(speed); // new velocity

    drive.runVelocity(new ChassisSpeeds(velocity.getX(), velocity.getY(), 0.0));

    // turret.followFieldCentricTarget(() -> robotPose.getTranslation().getAngle());
  }
